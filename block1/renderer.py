...
SOURCE_BADGES = {
    "Google Trends": "GT",
    "GitHub Trending (daily)": "GH-D",
    "GitHub Trending (weekly)": "GH-W",
    "GitHub Trending (monthly)": "GH-M",
    "Wikipedia Top": "WIKI",
}
...
def render_markdown(results: list[FetchResult], iso_str: str) -> str:
    today = datetime.now().strftime("%Y-%m-%d")
    lines = [
        f"# Trend Research Report — {today}",
        "",
        f"> Generated at: {iso_str}  ·  Sources: {len(results)}",
        "",
        "## Source Status",
        "",
    ]
        ...
        for i, it in enumerate(items, 1):
            ...
            if it.score is not None:
                if src.startswith("Wikipedia"):
                    score_part = f" · views={_fmt_num(it.score)}"
...
def _status_pill(ok: bool, text: str) -> str:
    cls = "ok" if ok else "fail"
    return f'<span class="{cls}">{escape(text)}</span>'


def _badge_for(src: str) -> str:
    for prefix, badge in SOURCE_BADGES.items():
        if src == prefix or src.startswith(prefix):
            return badge
    return ""


def render_html(results: list[FetchResult], iso_str: str) -> str:
    today = datetime.now().strftime("%Y-%m-%d")
    parts = [
        "<!DOCTYPE html>",
        '<html lang="en"><head><meta charset="utf-8">',
        '<meta name="viewport" content="width=device-width, initial-scale=1">',
        f"<title>Trend Research Report — {today}</title>",
        f"<style>{HTML_CSS}</style>",
        "</head><body>",
        "<h1>Trend Research Report</h1>",
        f'<div class="meta">Date: {today}  ·  Generated at {escape(iso_str)}  ·  Sources: {len(results)}</div>',
        '<div class="status-list"><strong>Source status:</strong> ',
    ]
    pills = []
    for r in results:
        text = f"{r.source}: {'OK' if r.ok else 'FAIL'} ({len(r.items) if r.ok else r.error})"
        pills.append(_status_pill(r.ok, text))
    ...
    if not grouped:
        ...
        for src, items in grouped.items():
            ...
            for i, it in enumerate(items, 1):
                ...
                if it.score is not None:
                    if src.startswith("Wikipedia"):
                        meta_bits.append(f"views {_fmt_num(it.score)}")
                    elif src.startswith("GitHub Trending"):
                        period = it.extra.get("stars_period", "today")
                ...
                lang_name = it.extra.get("language")
                lang_html = (
                    f'<span class="badge">{escape(lang_name)}</span>'
                    if lang_name
                    else ""
                )
                desc = it.extra.get("description") or ""
                desc_html = (
                    f'<div class="item-desc">{escape(desc[:200])}</div>'
                    if desc
                    else ""
                )
                url_html = (
                ...
                parts.append(
                    f'<div class="item">'
                    f'<div><span class="badge">{i}</span>{lang_html}'
                    f'<span class="item-title">{escape(it.title)}</span>'
                    f"{meta_html}</div>"
                    f"{desc_html}{url_html}"
                    f"</div>"
                )
            parts.append("</div>")

    parts.append(
        f'<div class="footer">Generated by trend research aggregator · {escape(iso_str)}</div>'
    )
